module functions
  ...
  abstract interface
    !
    real(rk) function analytical_fieldT (r,parameters)
      use accuracy
      !
  ...
  subroutine define_analytical_field(ftype,fanalytical_field)
    ...
    procedure (analytical_fieldT),pointer,intent(in) :: fanalytical_field => null()
    !
    select case(ftype)
      !
    case("MORSE")
    ...
    case("COSH-POLY") ! "Diabatic coupling as a polynom/cosh"
      ...
      !
    case("BOBLEROY","BOB") ! "BOB expansion"
      !
      fanalytical_field => poten_BOBLeRoy
      !
    case("BOBLEROY_DAMP","BOB_DAMP") ! "BOB expansion with damping to zero at r=0"
      !
 ...
 function poten_exponential(r, parameters) result(fun)
  ...
  !
  !
  function poten_fermi(r,parameters) result(fun)
    implicit none
    real(rk),intent(in) :: r,parameters(:)
    integer(ik)  :: m
    real(rk)            :: fun
    m=size(parameters)
    if(m/=8) stop 'poten_fermi: illegal number of parameters parameters (/=8)'
    fun = parameters(1)+parameters(2)/(parameters(3)+exp((-parameters(4)+parameters(6)*r**3+&
          parameters(7)*r**2+parameters(8)*r)/parameters(5)))
  end function poten_fermi
  !
