...
!
  ...
  real(rk)           :: memory_limit =   100000.0 ! GiB
  !
  type tim
   ...
   !
    ...
    subroutine ArrayStart(name,alloc,isize,ikind,hsize)
      ...
      if (alloc/=0) then
          ...
          write(out,"( ' Array dimension = ',i9,' array size =  ',f20.2,' GiB')") hsize_,size_
          call MemoryReport
    ...
    subroutine MemoryReport
      ...
      write (out,"(t2,'Active Arrays',t55,'size (GiB)')")
      !write (out,"()")
      !
      omitted = 0
      scan: do ord=1,array_count
        ...
        !  Output
        !
        write (out,"(t2,a,t50,t55,F11.6)") &
               t%name, t%size
      end do scan
      write (out,"(t2,'Total memory   = ',t47,f18.8,' GiB')") memory_now
      write (out,"(t2,'Maximal memory = ',t47,f18.8,' GiB (',f16.1,')')") maxmemory,memory_limit
