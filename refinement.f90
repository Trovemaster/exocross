module refinement
 ...
 subroutine sf_fitting
      ...
      integer(ik)  :: i,numpar,itmax,j,jlistmax,rank,ncol,nroots
      ...
      logical :: printed ! used to print frequencies 
       ...
       !
       nroots = (vmax+1)*Nestates*2*(2*nint(sigmai)+1)/2+1
       allocate (energy_(nrot,2,nroots),calc(nrot,2,nroots),stat=info)
       call ArrayStart('energy_',info,size(energy_),kind(energy_))
       !
       ! Allocate objects, that will be used for the fitting procedure:
       !
       allocate (rjacob(npts,parmax),eps(npts),deriv(nrot,2,nroots,parmax),stat=info)
       ...
       outer_loop: do while (still_run)  
          ...
          do while( fititer.le.itmax .and. stadev.ge.fitting%target_rms.and. stability.ge.stab_best)
            ...
            !
            if (action%frequency) then 
              char_fmt = "(/1X,215('-'),/'|  ## |  N |     J  p |  N |     J  p |      Obs.    |    Calc.  &
                         & | Obs.-Calc. |   Weight | State vib Lambda  Sigma Omega State vib Lambda Sigma &
                         &Omega    State vib Lambda  Sigma Omega  State vib Lambda Sigma Omega',/1X,215('-'))"
            else
              char_fmt = "(/1X,156('-'),/'| ## |  N |      J  p |      Obs.    |    Calc.   | Obs.-Calc. | &
                        &  Weight | States  vib  Lambda  Sigma  Omega  Spin  States   vib Lambda Sigma &
                        &  Omega  Spin ',/1X,156('-'))"
            endif 
            !
            write(out ,char_fmt)
            write(enunit,"(/1X,156('-'),/'| ## |  N |      J  p |      Obs.    |    Calc.   | Obs.-Calc. | & 
                         &  Weight | States  vib Lambda  Sigma  Omega  Spin | States vib Lambda Sigma & 
                         &  Omega  Spin ',/1X,156('-'))")
            !
            deriv_recalc = .true.
            ...
            if (action%frequency) then
              ...
              do iener = 1,en_npts
                 ...
                 !
                 write (out,"(i5,2(i5,1x,f8.1,1x,a1),2x,' ',3f13.4,2x,e9.2,2x,2f13.4,2x," &
                 ...
                 do k = max(i-5,1),min(i+5,nenergies(irot,itau))
                    ...
                    do k_ = max(i_-3,1),min(i_+3,nenergies(irot_,itau_))
                       ...
                       if (abs(fitting%obs(iener)%energy-(energy_(irot,itau,k)-energy_(irot_,itau_,k_)))& 
                           >real(maxiter_as,8)*abs(fitting%threshold_lock)) cycle
                       !
                       ...
                       !
                       write (frequnit,"(i5,2(i5,1x,f8.1,1x,a1),2x,' ',3f13.4,2x,e9.2,2x,2f13.4,2x,&
                              &'(',1x,i3,2x,2i4,2f8.1,' <-',1x,i3,2x,2i4,2f8.1,')" &
                              // "(',1x,i3,2x,2i4,2f8.1,' <-',1x,i3,2x,2i4,2f8.1,')',a)") &
                        ...
                        if (i==1.or.energy_(irot,itau,i)-ezero(1)>sqrt(small_)) then 
                           ...
                           do jener=1,en_npts
                             ...
                             if (fitting%obs(jener)%N ==i .and.fitting%obs(jener)%irot ==irot & 
                                ...
                                !
                                write(enunit,"(2i5,1x,f8.1,1x,a1,2x,' ',3f13.4,2x,e9.2,2x,'(',1x,i3,2x,2i4,3f8.1,')', &  
                           ...
                           if (.not.printed) then 
                                ...
                                !
                                write(enunit,"(2i5,1x,f8.1,1x,a1,2x,' ',3f13.4,2x,e9.2,2x,'(',1x,i3,2x,2i4,3f8.1,')',2x,a)") &
                      ...
                      do itau = 1,2
                        ...
                        loop_i_l : do i0 = k0,nenergies(irot,itau)
                          ...
                          if (abs( fitting%obs(iener)%energy-(energy_(irot,itau,i0)-ezero(1)) )<= & 
                                                      & real(iter_th,8)*abs(fitting%threshold_lock).and.&
              ...
              do iener = 1,en_npts
                 ...
                 ivibC   = calc(irot,itau,i)%v
                 !
                 write (out,"(2i5,1x,f8.1,1x,a1,2x,' ',3f13.4,2x,e9.2,2x,&
       ...
       !
       !
6552   format(/3X,85('-')/'   |  Iter  | Points | Params |    Deviat     |',&
       '    ssq_ener   |   ssq_pot   | Stability |'/&
       3X,85('-')/,&
       '   | ',i6,' | ',i6,' | ',i6,' |  ',E12.5,' | ',E12.5,'  |  ',&
            E10.3,' |',E10.3,' |',/3X,85('-')/)

6553   format(/3X,85('-')/'   |  Iter  | Points | Params |    Deviat     |',&
